...
function renderCaptures() {
  ...
  const captureBadge = (capture) => capture.seed ? "seed" : capture.automated ? "auto" : capture.storage === "github" || capture.library ? "library" : "browser";
  const captureTone = (capture) => capture.seed || capture.automated ? "info" : capture.storage === "github" || capture.library ? "good" : "warn";
  const canDelete = (capture) => !capture.seed && !capture.automated && capture.storage !== "github" && !capture.library;
  shell("Screenshot capture and review", `
    ...
    <div class="capture-layout"><div>
      ...
      <section class="panel"><div class="panel-header"><div><h3>Screenshot library</h3><p>Saved captures for ${esc(item.name)}.</p></div></div><div>${captures.length ? captures.map((capture) => `<article class="capture-row"><div><strong>${surfaceName(capture.surface)} ${esc(capture.device)} - ${esc(capture.period)}</strong><span>${fmt(capture.at)}${capture.notes ? ` - ${esc(capture.notes)}` : ""}</span></div>${badge(captureBadge(capture), captureTone(capture))}<button class="icon-button" data-delete="${capture.id}" ${canDelete(capture) ? "" : "disabled"}>X</button></article>`).join("") : `<p class="empty-state">No screenshots saved for this retailer yet.</p>`}</div></section>
    </div><aside class="panel"><div class="panel-header"><div><h3>Add screenshot</h3><p>Captures save to the shared screenshot library when available.</p></div></div><div class="panel-body"><div class="field-grid" style="grid-template-columns:1fr"><label class="field">Period<input id="uploadPeriod" value="${esc(state.uploadPeriod)}"></label><label class="field">Screen<select id="uploadSurface"><option value="cart" ${state.surface === "cart" ? "selected" : ""}>Cart</option><option value="checkout" ${state.surface === "checkout" ? "selected" : ""}>Checkout</option></select></label><label class="field">Device<select id="uploadDevice"><option value="desktop" ${state.device === "desktop" ? "selected" : ""}>Desktop</option><option value="mobile" ${state.device === "mobile" ? "selected" : ""}>Mobile</option></select></label><label class="field">PM notes<textarea id="notes" placeholder="Fulfillment, promo, subscription, payment, or friction notes."></textarea></label></div><input id="fileInput" type="file" accept="image/png,image/jpeg,image/webp" hidden><div class="drop-zone" id="dropZone" tabindex="0" aria-label="Paste or drop screenshot"><div><strong>Paste screenshot</strong><span>Clipboard, PNG, JPEG, or WebP</span></div></div><button class="primary-action" id="saveShot" style="width:100%;margin-top:12px">Choose file</button></div></aside></div>`);
  bindCaptureControls();
}
